...
\makeatletter % to use \protected@edef, else make first a no @ alias
...
%% I see in the ngpdf.com editor
  \protected@edef\treenode{node {%
\unless\ifdefined\NOTAGGING
                                 \etocthetocitembegintag
\fi
                                 \parbox{2.5cm}{%
                                   \Centering
                                   \textbf{\etocthelinkedname}
...
\ifdefined\NOTAGGING
    (\textit{\etocthelinkedname})
\else
                                   (\textit{\etoctagLbl{\etocthelinkednumber}})
\fi
