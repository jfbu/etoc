...
\ifdefined\NOTAGGING\let\preparetreenodeastaggedtocitem\empty\fi
\makeatother
...
\etocsettocstyle
  {\treetok{\node {\unless\ifdefined\NOTAGGING
\tagstop %% TODO: THINK OF A BETTER WAY TO HANDLE THIS
\fi
                   \parbox{2cm}{\raggedright THE eTOC AS A TIKZ MOLECULE}%
                   \unless\ifdefined\NOTAGGING
\tagstart
\fi
                   }%
           }%
...
\unless\ifdefined\NOTAGGING\etoctagstartTOCcontents\fi
   \the\treetok
\unless\ifdefined\NOTAGGING\etoctagfinishTOCcontents\fi
   \end{tikzpicture}%
}
